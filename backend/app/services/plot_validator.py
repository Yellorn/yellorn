...
class PlotValidator:
    ...
    def _validate_code_sections(
        self, code_data: Dict[str, Any], result: Dict[str, Any]
    ):
        ...
        for lang, code in code_data.items():
            ...
            for pattern in dangerous_patterns:
                if re.search(pattern, code, re.IGNORECASE):
                    result["errors"].append(
                        f"Potentially dangerous code pattern found in {lang}: {pattern}"
                    )

            # Check code length
            if len(code) > 50000:  # 50KB limit per code section
                result["warnings"].append(
                    f"Code section for {lang} is very large ({len(code)} chars)"
                )
    ...
    def _validate_performance(self, plot_data: Dict[str, Any], result: Dict[str, Any]):
        ...
        if "particle_count" in style:
            ...
            if isinstance(count, int):
                if count > 50000:
                    result["errors"].append(
                        "Particle count exceeds maximum limit (50,000)"
                    )
                elif count > 10000:
                    result["warnings"].append(
    ...
    def _validate_ai_compatibility(
        self, plot_data: Dict[str, Any], result: Dict[str, Any]
    ):
        ...
        if "interactions" not in plot_data or not plot_data["interactions"]:
            result["suggestions"].append(
                "Consider adding interaction definitions for better AI engagement"
            )

        if "metadata" not in plot_data or not plot_data["metadata"]:
            result["suggestions"].append(
                "Consider adding metadata for better discoverability"
            )
